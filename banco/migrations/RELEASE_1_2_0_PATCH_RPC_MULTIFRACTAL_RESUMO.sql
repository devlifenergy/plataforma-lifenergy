-- RELEASE 1.2.0 - Correção definitiva do envio multifractal e do campo initial_time
-- Execute este arquivo no Supabase SQL Editor.
-- A função antiga da 1.1.10 permanece intacta.
-- A função v2 anterior também permanece; esta release passa a usar uma função nova,
-- com nome exclusivo, para evitar conflito/overload no PostgREST.

-- Remove somente versões anteriores desta NOVA função, se já existirem.
do $$
declare
  r record;
begin
...
begin
  ...
  -- Conversão explícita: nunca envia texto diretamente para a coluna initial_time.
  v_initial_time := case
...
-- Verificação rápida: deve retornar 1 linha.
select
  p.proname as funcao,
