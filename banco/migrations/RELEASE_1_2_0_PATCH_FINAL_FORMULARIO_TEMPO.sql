-- RELEASE 1.2.0 - Patch final de formulário, resumo por fractal e conversão de hora
-- Execute este arquivo no Supabase SQL Editor após a migration principal da 1.2.0.
-- Este patch NÃO altera a versão estável 1.1.10; ele recria somente a função v2.

-- Remove com segurança qualquer versão anterior da função v2, inclusive possíveis overloads.
do $$
declare
  r record;
begin
  for r in
    ...
    where n.nspname = 'public'
      and p.proname = 'submit_public_journey_response_v2'
  loop
...
create function public.submit_public_journey_response_v2(
  p_token text,
